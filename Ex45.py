...
class Fight(object):
    ...
    def FightWon(Fight, Player, Enemy):
        printborder()
        ...
        raw_input("Press Enter")
        if xpLeft== 0:
            Fight.LevelUp( Player)
        else:
            basicfight.FightStart(player1, EnemyBasic(100, 100, 5, 10, 20, 20, False, "Basicenemy")) #TESTING


    def LevelUp(Fight, Player):
...
def berserk(Player, Enemy):
    global oldarmor, oldattackLow, oldattackHigh, BerserkDuration
    BerserkDuration = 0
    if Player.attackLow > oldattackLow:
        print "You are already in Berserk mode! Your energy is wasted."
        raw_input("Press Enter")
        return
    oldarmor = Player.armor
    oldattackLow = Player.attackLow
    oldattackHigh = Player.attackHigh
    Player.armor = 0
